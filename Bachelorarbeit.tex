...
\\
b = u >> 1;   & MAX = getMaximalFrequency();MAX >> 1;
\\
{\tt{return}} u-{\tt{x}} > {\tt{x}} - b ? b : u; & {\tt{ELSE}}
\\
END FUNCTION & MIN = getMinimalFrequency();MIN << 1;
\\
& {\tt{END WHILE}}
\\
& END FUNCTION
...
\par
Der entgegenzunehmende Formatstring hat folgende Gestalt:
\begin{verbatim}
Pfad Vorkonditionierer Kompressor Iterationsweg 
\end{verbatim}
\begin{itemize}
	\item {\tt{Pfad}}: Absoluter Pfad zu Bilddatei
	\item {\tt{Vorkonditionerer}}: $n\in \{0,\ldots, 10\}$
	\\
	Dabei entsprechen die Ganzzahlen den in Kapitel 3 besprochenen Filtern.
	\item {\tt{Kompressor}}: $n\in\{1,2\}$
	\\
	Das {\tt{rans}}-Verfahren wird durch $n=0$ gewählt, dass {\tt{uans}}-Verfahren durch $n=1$.
	\item {\tt{Iterationsweg}}: $n\in\{0,1,2\}$
	\\
	Zeilenweise Iteration bei der Kompression wird durch $n=0$ gewählt, spaltenweise durch $n=1$ und kanalweise durch $n=2$.
\end{itemize} 
\newpage
