...
\end{Beispiel}
\begin{Beispiel}
Für die Demonstration eines konkreten Beispiels wird die auf dem drei-Symbol Alphabet  $\A :=\left\{a, b, c\right\}$    mit $p_a=1 /2, p_b = 1/4, p_c = 1/4$ bestehende Nachricht "`abbac"' für $b=2,\;L=16$ kodiert und dekodiert. Sei
\begin{align*}
\C(a,x)&=2 x
\\
\C(b,x) &= 4 x + 1
...
\C(c,x) &= 4 x + 3.
\end{align*}
Dann erhält man durch einfaches Rechnen die den Symbolen korrespondierenden $b$-eindeutigen Index-Mengen:
\par
\vspace{0.5cm}
\begin{center}
 \begin{tabular}{l|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c}
...
\end{center}
\vspace{0.5cm}
Offensichtlich lässt sich folgende Inverse definieren:
$$
\D(x):
               \begin{cases}
                ...
                \end{cases}
$$
Dabei gilt: Wird $u\in\A$ mit $\C$ codiert wächst $x$ um den Faktor $X = 1 / p_u$. Für $L=16$ und $b=2$ erhält man das normalisierte Interval $I=\left\{16, 17,\ldots,31\right\}$.
